...
`POST`-body support currently covers `/api/v2/products`, `/api/v2/warehouse-transfers`, `/api/v2/inventory-adjustments`, `/api/v2/inventory-movements`, `/api/v2/fifo-layers`, `/api/v2/backorder-queues`, `/api/assemblies`, and `/api/stock-takes/list`; other list endpoints take the base64 `GET` form.
